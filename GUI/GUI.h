...
private:
	...
	static const int	
		...
		MenuItemWidth = 75,		//Width of each item in menu bar menu
...
public:
	...
	MENUITEM GetUserAction(int x=0,int y=0,bool check=true) const;

	// Output Functions  ---------------------------
	void PrintMessage(string msg, int pos = 0) const; // prints a message in the status bar

	void DrawUnPavedDis(Castle &c);
